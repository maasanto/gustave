...
def generate_qa_pairs_with_llm(section_text, index):
    """
    Uses an LLM with llama-index to generate Q&A pairs for a given section of text.
    """
    
    # Query the index to generate Q&A
    prompt = "Generate multiple question and answer based on the \n\n{section_text}. The questions should be written from the point of a view of a user on a support. Use the following format QUESTION: content/nANSWER: content"
...
def process_file_with_llm(file_path, index):
    """
    ...
    for section in sections:
        if section.strip():  # Avoid empty sections
            qa_pair = generate_qa_pairs_with_llm(section.strip(), index)
            qa_pairs.append(qa_pair)

    return qa_pairs

def scan_directory_with_llm(root_dir, file_extensions=('.md', '.txt', '.py'), index=[]):
    """
    ...
    for dirpath, _, files in os.walk(root_dir):
        for file_name in files:
            ...
            if file_name.endswith(file_extensions):
                ...
                qa_pairs = process_file_with_llm(file_path, index)
...
if __name__ == "__main__":
    ...
    data_dir = "/Users/tat/dev-local/data_sample"  # replace with your directory
    output_file = "qa_pairs.json"

    # Load the index
    index = load_index(f"index-{data_dir}", data_dir)
    
    # Scan the directory and extract Q&A pairs
    qa_pairs = scan_directory_with_llm(data_dir, ('.md', '.txt', '.py'), index)
